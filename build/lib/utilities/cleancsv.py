...
def get_columns_to_copy(headers):
    selected_columns = input(
        colored(
            "Enter the columns you want to copy separated by commas, press Enter to include all columns: ",
            "light_yellow",
        )
    )
...
def main():
    print(colored("\nCleanCSV\n---------", "blue"))
    input_file = input(
        colored("Enter the location of your CSV-file: ", "light_yellow")
    ).strip('"')
    ...
    convert = input(
        colored(
            f"Do you want to convert {output_file} to Excel? (y=yes): ", "light_yellow"
        )
    )
